...
def input_guess(grid_size: int, specification: str) -> int:
    """This is a docstring about asking the user to make guesses!"""
...
def print_grid(grid_size: int, guessr: int, guessc: int, correct: bool) -> None:
    """Docstring with instructions for printing emoji square grid!"""
    row_counter = 1
...
def correct_guess(secretr: int, secretc: int, guessr: int, guessc: int) -> bool:
    """Function to determine if the guessed location and secret location match."""
    return secretr == guessr and secretc == guessc
# main function that compiles all the previous functions to streamline the game


def main(grid_size: int, secretr: int, secretc: int) -> None:
    """Function to compile all the prior functions into one cohesive game!"""
